...
import NotificationSseListener from '@/components/pwa/notification-sse-listener';
...
export default function MainHeader() {
  ...
  }, [isNotificationPanelOpen, refreshNotifications]);
  ...
  const handleReadAllNotifications = async () => {
    if (unreadCount <= 0 || isReadAllPending) return;
            ...
            >
              ...
              <div className="px-4 py-4">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      className="rounded-md px-2 py-1 text-lg leading-none font-medium text-[#667085] transition-colors hover:bg-[#EAF3F1] disabled:cursor-not-allowed disabled:opacity-50"
                      disabled={unreadCount <= 0 || isReadAllPending}
                      onClick={handleReadAllNotifications}
                    >
                      {isReadAllPending ? '처리 중...' : '모두 읽음'}
                    </button>
                    <button
                        ...
                        <Link
                          ...
                          onClick={() => {
                            ...
                            if (Number.isFinite(notificationId)) {
                              void readNotification(notificationId)
                                .then((result) => {
                                  if (!result.success) {
